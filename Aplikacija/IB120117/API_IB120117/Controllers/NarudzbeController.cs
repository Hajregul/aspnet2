...
namespace API_IB120117.Controllers
{
    public class NarudzbeController : ApiController
    {
        ...
        [HttpGet]
        [Route("api/Narudzbe/AllIsporuceni/{status}")]
        public List<esp_AllNarudzbeStatus_Result> AllIsporuceni(bool status)
        {
            return db.esp_AllNarudzbeStatus(status).ToList();
        }
        ...
        public List<esp_Narudzbe_DateOdDateDo_Result> Report(int klijentID, bool status,DateTime? datumOd, DateTime? datumDo)
        {
            return db.esp_Narudzbe_DateOdDateDo(klijentID, status, datumOd, datumDo).ToList();
        }
        [HttpGet]
